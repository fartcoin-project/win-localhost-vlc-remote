...
$(function () {
    $('#albumArt').load(function () {
        var width = (2.8 * $(this).width());
        var height = (2.8 * $(this).height());
        ...
        $(this).attr('width', width).attr('height', height).css('margin-left', Math.floor((350 - width) / 2)).css('margin-top', Math.floor((300 - height) / 2)).removeClass('hidden').fadeIn();
    });
    $('#libraryTree').jstree({
        "xml_data": {
            "ajax": {
